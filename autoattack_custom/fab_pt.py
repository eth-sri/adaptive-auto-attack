...
class FABAttack():
    ...
    def attack_single_run_targeted(self, x, y, use_rand_start=False):
        """
        :param x:    clean images
        :param y:    targeted labels
        """
        ...
        y_pred = self._get_predicted_label(x)
        # if y is None:
        clean = y_pred.detach().clone().long().to(self.device)
        # else:
        # y = y.detach().clone().long().to(self.device)
        pred = y_pred != y
        corr_classified = pred.float().sum()
        ...
        la_target = y  # output.sort(dim=-1)[1][:, -self.target_class]
        
        startt = time.time()
        # runs the attack only on correctly classified points
        im2 = x[pred].detach().clone()
        la2 = clean[pred].detach().clone()
    ...
    def perturb(self, x, y):
        ...
        with torch.no_grad():
            ...
            if isinstance(y, TargetedMisclassification):
                ...
                targeted = True
            elif isinstance(y, Misclassification):
                y = y.labels.raw
