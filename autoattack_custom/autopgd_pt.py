...
class APGDAttack():
    def __init__(self, model, n_iter=100, norm='Linf', n_restarts=1, eps=None,
                 seed=0, loss='ce', eot_iter=1, rho=.75, verbose=False, mod=None,
                 device='cuda'):
        self.model = model
        self.n_iter = n_iter
        self.eps = eps
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        targeted = False
        if isinstance(y_in, TargetedMisclassification):
            y_in = y_in.target_classes.raw
            targeted = True
        elif isinstance(y_in, Misclassification):
            y_in = y_in.labels.raw
        ...
        mod = {'indiv': 1, 'logits': 1}
        if self.loss == "logit":
            match_target = extract_target_logits(self.model, x_in, y_in)
            mod.update({"match_target": match_target})
        mod.update(self.mod)
        loss_fn = get_loss_fn(self.model, y, self.loss, targeted, mod)

        x_adv.requires_grad_()
        grad = torch.zeros_like(x)
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                loss, loss_indiv, logits = loss_fn(x_adv)
            ...
            if not cheap:
                ...
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        if targeted:
                            y_to_fool = TargetedMisclassification(y_to_fool)
