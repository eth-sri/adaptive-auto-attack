...
class Linf_extended_search_space(search_space):
    def __init__(self, flags=None):
        super().__init__(flags)
    ...
    def define_space(self, config, defense_property):
        if "use_loss" in self.flags and not self.flags["use_loss"]:
            ...
            set_targeted = hp.choice('set_targeted', [True, False])
            loss = hp.choice('loss', ['ce', 'dlr', 'hinge', 'l1', 'zero1', 'zero2', 'zero3', 'zero4'])
            loss_nologit = hp.choice('loss_nologit', ['ce', 'dlr', 'hinge', 'l1'])
            use_softmax = hp.choice('use_softmax', [True, False])
            use_loss_diff = hp.choice('loss_diff', [True, False])
        n_target_classes = hp.randint('n_target_classes', 1, 9)
        ...
        BB_steps = hp.randint('BB_steps', 20, 100)
        BB_momentum = hp.uniform('BB_momentum', 0, 1)
        D_a = hp.uniform('D_a', 0, 10)
        D_b = hp.uniform('D_b', 0, 10)
        D_c = hp.uniform('D_c', 0, 10)
        ...
        BB_dict = {'attack': LinfBrendelBethgeAttack(),
                   ...
                   "lr_num_decay": 20,
                   "momentum": BB_momentum, }
        D1 = {'attack': LinfDummy1(),
              'loss': loss,
              "a": D_a,
              "b": D_b,
              "c": D_c}
        D2 = {'attack': LinfDummy2(),
              'loss': loss,
              "a": D_a,
              "b": D_b,
              "c": D_c}
        D3 = {'attack': LinfDummy3(),
              'loss': loss,
              "a": D_a,
              "b": D_b,
              "c": D_c}
        D4 = {'attack': LinfDummy4(),
              'loss': loss,
              "a": D_a,
              "b": D_b,
              "c": D_c}
        D5 = {'attack': LinfDummy5(),
              'loss': loss,
              "a": D_a,
              "b": D_b,
              "c": D_c}
        D6 = {'attack': LinfDummy6(),
              'loss': loss,
              "a": D_a,
              "b": D_b,
              "c": D_c}
        D7 = {'attack': LinfDummy7(),
              'loss': loss,
              "a": D_a,
              "b": D_b,
              "c": D_c}
        D8 = {'attack': LinfDummy8(),
              'loss': loss,
              "a": D_a,
              "b": D_b,
              "c": D_c}
        attack_space = [FGSM_dict, PGD_dict, DF_dict, CW_dict, FAB_dict, SQR_dict, NES_dict, D1, D2, D3, D4, D5, D6, D7, D8]
