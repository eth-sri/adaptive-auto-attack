...
from attack.criteria import Criterion
...
from attack.loss import get_loss_fn
...
class DeepFoolAttack(MinimizationAttack, ABC):
    ...
    #
    #         def loss_fun(
    #             inputs: ep.Tensor, k: int
    #         ) -> Tuple[ep.Tensor, Tuple[ep.Tensor, ep.Tensor]]:
    #             import numpy as np
    #             x = model(inputs.raw)
    #             x_sorted, ind_sorted = x.sort(dim=1)
    #             ind = (ind_sorted[:, -1] == k).float()
    #             loss = -(x[np.arange(x.shape[0]), k] - x_sorted[:, -2] * ind - x_sorted[:, -1] * (
    #                         1. - ind)) / (x_sorted[:, -1] - x_sorted[:, -3] + 1e-12)
    #             loss = ep.astensor(loss)
    #             return loss.sum(), (loss, ep.astensor(ep.astensor(x)))
    #
    #     else:
    #         raise ValueError(
    #             f"expected loss to be 'logits' or 'crossentropy' or 'dlr', got '{self.loss}'"
    #         )
    #
    #     return loss_fun
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        mod_dict = {'indiv': 1, 'logits': 1, 'loss_diff': True}
        mod_dict.update(self.mod)
        def make_loss(model, targets):
            loss_fn_raw = get_loss_fn(model=model, labels=targets, type=self.loss, targeted=True, modifier=mod_dict)

            def loss_fn(x):
                result = loss_fn_raw(x)
        ...
        for i in range(candidates):
            targets = classes[:, i]
            loss_fn_target = make_loss(model, targets)
            loss_list.append(ep.value_and_grad_fn(x, loss_fn_target, has_aux=True))
        ...
        for _ in range(self.steps):
            ...
            diffs += [loss_list[k](x) for k in range(2, candidates)]
